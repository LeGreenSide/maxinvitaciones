const WhatsAppButton = ({
  invitationTitle,
  detalle,
  fullWidth = false,
  compact = false,
  label,
  className = '',
}) => {
  const phoneNumber = '56987401906';
  const message = invitationTitle
    ? `Hola! Quiero la invitación de "${invitationTitle}"${detalle ? ` (${detalle})` : ''} 🎉`
    : 'Hola! Quiero cotizar una invitación digital 🎉';

  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;

  return (
    <a
      href={whatsappUrl}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center gap-2 rounded-full bg-linear-to-b from-[#12b05a] to-[#0b8a45] font-semibold text-white shadow-[0_4px_12px_rgba(18,176,90,0.35)] transition-all duration-200 hover:-translate-y-0.5 hover:from-[#26e084] hover:to-[#12b05a] hover:shadow-[0_10px_24px_rgba(18,176,90,0.55)] active:translate-y-0 ${
        compact
          ? 'h-11 px-4 text-[13px] sm:h-12 sm:text-[15px] lg:h-[52px] lg:text-[17px]'
          : 'h-11 px-5 text-[15px] sm:h-12 sm:text-[16px]'
      } ${fullWidth ? 'w-full justify-center' : ''} ${className}`}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        className={
          compact
            ? 'h-4 w-4 shrink-0 sm:h-[18px] sm:w-[18px] lg:h-5 lg:w-5'
            : 'h-5 w-5 shrink-0'
        }
        fill="currentColor"
        aria-hidden="true"
      >
        <path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38c1.45.79 3.08 1.21 4.79 1.21 5.46 0 9.91-4.45 9.91-9.91S17.5 2 12.04 2zm0 18.15c-1.48 0-2.93-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.15 8.15 0 01-1.25-4.38c0-4.54 3.7-8.24 8.24-8.24 2.2 0 4.27.86 5.83 2.42a8.19 8.19 0 012.41 5.83c0 4.54-3.7 8.23-8.24 8.23zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.17.25-.64.81-.78.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-2-1.23-.74-.66-1.24-1.47-1.39-1.72-.14-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.12-.14.16-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.41-.42-.56-.43-.14-.01-.31-.01-.48-.01-.17 0-.43.06-.66.31-.23.25-.86.85-.86 2.07 0 1.22.89 2.4 1.01 2.56.12.17 1.75 2.67 4.25 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.67-1.18.21-.58.21-1.08.14-1.18-.06-.11-.23-.17-.48-.29z" />
      </svg>
      {label ?? 'Pedir'}
    </a>
  );
};

export default WhatsAppButton;
